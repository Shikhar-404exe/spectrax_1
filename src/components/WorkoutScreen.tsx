...
const getDefaultPanelPositions = (): PanelPositions => {
  ...
  return {
    ...
    reps: {
      x: Math.max(width / 2 - 110, 30),
      y: Math.max(height - 250, 30),
    },
...
export const WorkoutScreen: React.FC<WorkoutScreenProps> = ({
  ...
  const panelRefs =
    useRef<Record<WorkoutPanelId, React.RefObject<HTMLDivElement>> | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      wsSocket.onopen = () =>
        console.log("[SpectraX WS] connected to backend");
      wsSocket.onerror = () => {
        wsSocket = null;
